...
namespace YoElectronicShop.Services
{
    public class ShoppingCartService : IShoppingCartService
    {
        ...
        public async Task<List<CartItemDto>> GetItems(int userId)
        {
            try
            {
                ...
                else
                {
                    ...
                    var message = await response.Content.ReadAsStringAsync();
                    throw new Exception($"Htttp status code: {response.StatusCode} Message: {message}");
                }
        ...
        public async Task<CartItemDto> UpdateQty(CartItemQtyUpdateDto cartItemQtyUpdateDto)
        {
            try
            {
                var jsonRquest = JsonConvert.SerializeObject(cartItemQtyUpdateDto);
                var content = new StringContent(jsonRquest,Encoding.UTF8, "application/json-patch+json");
